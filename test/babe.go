...
func main() {
	sp, err := spider.CreateNewDomSpider("http://www.greatnudemodels.com", "span.galprov>a>img", "src")
	if err != nil {
...
func defaultLinkGenerator(page string, document string) ([]string, error) {
	...
	for _, v := range matches {
		...
		if strings.HasPrefix(link, "http://") || strings.HasPrefix(link, "https://") {
			if !strings.Contains(link, "js") && !strings.Contains(link, "css") && !strings.Contains(link, "jpg") && !strings.Contains(link, "png") && !strings.Contains(link, "gif") && !strings.Contains(link, "jpeg") && !strings.Contains(link, "xml") && !strings.Contains(link, "less") && !strings.Contains(link, "php") && !strings.Contains(link, "aspx") && !strings.HasSuffix(link, "ico") {
